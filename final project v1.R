...
param <- heemod::modify(
  ...
  cW_hpv = ifelse(((age_cycle > age_start_hpv & age_cycle <70) &model_time %% 3),110, 0), ## cost of well state (including testing) - HPV)
  eW_pap=ifelse(((age_cycle > age_start_pap & age_cycle <70) &model_time %% 3),.9, 1),  ## cost of well state (including testing) - pap
  eW_hpv = ifelse(((age_cycle > age_start_hpv & age_cycle <70) &model_time %% 3),.99, 1) ## cost of well state (including testing) - HPV)
)
...
## states are U=unhealed, H=healed, I=infected, Maj1st=major amp first year, Maj=major amp following years, Min1st=minor amp first year, Min=minor amp following years, D=death
...
## note: "method" is where the transition occurs, if ="beginning" it is at the beginning, if ="end" it is at the end and if ="life-table" then it is half-cycle corrected.
## also note: results are not exactly like Lindsey's because the calculation for half-cycle correction methodology for life-table vs. Treeage.
...
res_dsa_data <- res_dsa[["dsa"]]
res_dsa_data <- res_dsa_data %>% group_by(.par_names, .par_value) %>% mutate(incremental_cost = cost - lag(cost), incremental_effect = effect-lag(effect))
##hpv - pap
res_dsa_data1 <- res_dsa_data %>% remove_missing()
res_dsa_data1 <- res_dsa_data1 %>% mutate(icer = (incremental_cost/495777) / (incremental_effect/495777))

res_dsa_data1 <- res_dsa_data1 %>% mutate(.par_names = case_when(
  .par_names == "eW_pap" ~ "Pap Utility",
  .par_names == "eW_hpv" ~ "HPV Utility",
  .par_names == "eICC1" ~ "Local Cancer Utility",
  .par_names == "eICC2" ~ "Advanced Cancer Utility",
  .par_names == "eCIN23" ~ "CIN23 Utility",
  .par_names == "eCIN1" ~ "CIN1 Utility",
  .par_names == "cW_pap" ~ "Pap Test Cost",
  .par_names == "cW_hpv" ~ "HPV Test Cost",
  .par_names == "cRem" ~ "Remission Cost",
  .par_names == "cCIN1" ~ "CIN1 Cost",
  .par_names == "cCIN23" ~ "CIN23 Cost",
  .par_names == "cICC1" ~ "Local Cancer Cost",
  .par_names == "cICC2" ~ "Advanced Cancer Cost",
  .par_names == "age_base" ~ "Age Starting Test",
  
))


res_dsa_data2 <- subset(res_dsa_data1, icer >= 0)
res_dsa_data3 <- subset(res_dsa_data1, icer < 0)

dat<- res_dsa_data1 %>% select(.par_names, .par_value, icer)
dat$value <- rep_len(1:2, length.out=length(dat$icer))

ggplot(dat, aes(x= as.factor(.par_names), y = icer, fill = as.factor(value))) +
...
ggplot(res_dsa_data1, aes(x= as.factor(.par_names), y = icer)) +
  geom_bar(data =subset(res_dsa_data1, icer >= 0), aes(x = as.factor(.par_names), y = icer), stat = "identity")+
  geom_bar(data =subset(res_dsa_data1, icer < 0), aes(x = as.factor(.par_names), y = icer), stat = "identity")+
  coord_flip()+
  xlab("Value")+
  ylab("ICER")+
...
ggsave("C-e plot psa2.png", gridExtra::arrangeGrob(plot_1, plot_2, height = 9, width = 9))
